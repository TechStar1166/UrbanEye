...
export default function App() {
  ...
  const reset = () => { setPin(undefined); setAddressNote(''); setShowStorefronts(false); setGroups(ALL_GROUPS); setMetric('population'); setOpacity(75); setIncome(75); setCohort('25'); setResetKey(k => k + 1); };
    ...
    <main className={'workspace pane-' + pane}>
      ...
      <section className="map-workspace" aria-label="Map and layers">
        {view === 'map' ? <div className="map-viewport">
          {areas && <CommunityMap areas={areas} metric={metric} selectedId={selected?.geo_id} onSelect={select} opacity={opacity / 100} resetKey={resetKey} places={places} storefronts={visibleStorefronts} highlightIds={highlightIds} pin={pin} answerGeoId={answer?.mode === 'facts' && answer.evidence.length ? selected?.geo_id : undefined} />}
