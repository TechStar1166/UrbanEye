...
export function CommunityMap({ areas, metric, onSelect }: {
  areas: Areas; metric: string; onSelect: (id: string) => void;
}) {
  ...
  useEffect(() => {
    ...
    const polygons = L.geoJSON(areas as unknown as GeoJsonObject, {
      style: { color: metric === 'housing_units' ? '#8f4bb8' : '#087e8b',
        weight: 3, fillOpacity: metric ? 0.35 : 0 },
