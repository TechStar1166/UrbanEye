...
test('CSV export lists every area and metric with its source', async ({ page }) => {
  ...
  const rows = areas.features.reduce((sum: number, f: { properties: { metrics: object } }) => sum + Object.keys(f.properties.metrics).length, 0);
  expect(lines.length).toBe(1 + rows); // one row per area and metric
