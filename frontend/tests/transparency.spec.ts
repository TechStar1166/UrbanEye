...
test('CSV export lists every area and metric with its source', async ({ page }) => {
  ...
  expect(lines.length).toBe(1 + 3 * 2); // 3 areas x (population, housing_units)
...
test('an unknown area or tab in the link is ignored', async ({ page }) => {
  ...
  await expect(page.getByRole('complementary', { name: 'Area facts and evidence' }).getByRole('heading', { name: 'Silver Spring CDP' })).toBeVisible();
  await expect(page.getByRole('tab', { name: 'Overview' })).toHaveAttribute('aria-selected', 'true');
});

test('the address bar follows the selected area and tab', async ({ page }) => {
  await page.goto('/');
  await page.getByRole('textbox', { name: 'Search addresses or areas' }).fill('240317025011');
