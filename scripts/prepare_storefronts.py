...
STUDY_AREA_IDS = {"240317025011", "240317025021"}


def load_study_areas():
    """Keep the storefront query and assignments scoped to the original Fenton snapshot."""
    areas = json.loads(AREAS.read_text())
    areas["features"] = [f for f in areas["features"] if f["properties"]["geo_id"] in STUDY_AREA_IDS]
    if {f["properties"]["geo_id"] for f in areas["features"]} != STUDY_AREA_IDS:
        raise ValueError("Missing Fenton study block groups")
    return areas
